...
def del_check():
    ...
    if(pos == -1):
        ...
        if(w1[3] == 'N' | 'P'):
            tkinter.messagebox.showinfo(
                "Delete", "Clothes currently borrowed/Purchased. Please try another Clothes")
            del_menu.destroy()
            return(del_clothes)
...
def buy_check():
    ...
    pos = binary_search('Bindex.txt', bclothes)
    if pos == -1:
        tkinter.messagebox.showinfo(
            "Borrow", "The cloth ID that you had entered is not in our database,sorry,please enter a different Cloth ID")
        borrow_menu.lift()
    else:
        f2 = open('BData.txt', 'r+')
        f2.seek(pos)
